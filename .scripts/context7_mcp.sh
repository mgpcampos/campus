...
DEFAULT_ENCRYPTION_KEY="000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"
USER_AGENT="${SERVER_NAME}/${SERVER_VERSION}"

API_KEY="ctx7sk-cfa42198-1ff0-40b9-89b1-dd3c7bdbd50f"
...
print_help() {
  cat <<'EOF'
Context7 MCP server (Bash edition)
Usage:
  context7_mcp.sh [global options]                # Run as MCP stdio server (default)
...
while (($#)); do
  case "$1" in
    ...
    --)
      ...
      break
      ;;
    resolve|resolve-library-id|get-library-docs|get-docs|docs|mcp|serve)
      MODE="$1"
      shift
      break
      ;;
