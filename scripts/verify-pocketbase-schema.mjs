...
async function main() {
	...
	const mismatchedContent = mirroredFiles.filter((file) => migrationHashes.get(file) !== backupHashes.get(file));

	const summary = {
		status: missingInBackup.length === 0 && extraInBackup.length === 0 && mismatchedContent.length === 0 ? 'ok' : 'error',
