...
vi.mock('../pocketbase.js', () => {
  type Report = { id?:string; reporter?:string; targetType?:string; targetId?:string; reason?:string; status?:string };
  ...
  return { pb: { authStore: { model: { id: 'user1' } }, collection: (name:string) => {
    if (name === 'reports') return { create, getOne: vi.fn(async () => ({ id:'r1', targetType:'post', targetId:'p1', reporter:'user1' })), update };
    if (name === 'posts') return { getOne };
    if (name === 'moderation_logs') return { create: vi.fn() };
    if (name === 'comments') return { getOne: vi.fn(async () => ({ id:'c1', post:'p1', author:'u2' })) };
    return { getOne: vi.fn(), create: vi.fn(), update: vi.fn() };
  } } };
});

vi.mock('./permissions.js', () => ({ canModeratePost: vi.fn(async () => true), canModerateComment: vi.fn(async () => true) }));
