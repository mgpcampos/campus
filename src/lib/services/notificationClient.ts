...
export async function loadInitialNotifications(limit = 20) {
  ...
  if (!cu) return;
  try {
  const list = await pb.collection('notifications').getList(1, limit, { sort: '-created', expand: 'actor,post,comment' });
...
export async function subscribeNotifications() {
  ...
  unsub = await pb.collection('notifications').subscribe('*', (e: { action: string; record: NotificationRecord }) => {
...
export async function markRead(id: string) {
  try { await pb.collection('notifications').update(id, { read: true }); } catch (e) { console.warn('markRead failed', e); }
}
...
export async function markAllRead() {
  ...
  for (const n of list) {
    if (!n.read) {
      try { await pb.collection('notifications').update(n.id, { read: true }); } catch { /* ignore */ }
    }
  }
}
