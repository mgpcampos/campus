...
export async function getUserByUsername(pb: PocketBase, username: string): Promise<UserSummary | null> {
...
export async function listUserPosts(pb: PocketBase, userId: string, opts: ListUserPostsOptions = {}): Promise<{ items: UserPostSummary[]; page: number; perPage: number; totalPages: number; totalItems: number; }>{
...
export async function listUserMemberships(pb: PocketBase, userId: string): Promise<MembershipSummary>{
  // Spaces
  const spaceMemberships = await pb.collection('space_memberships').getFullList({
    filter: `user = "${userId}"`,
    expand: 'space'
  });
  interface MembershipRecord<T> { expand?: { [k: string]: T } }
  interface SpaceRecord { id: string; name: string; slug: string }
  ...
  interface GroupRecord { id: string; name: string; space: string }
...
export async function updateUserProfile(pb: PocketBase, userId: string, data: UpdateUserProfileInput){
