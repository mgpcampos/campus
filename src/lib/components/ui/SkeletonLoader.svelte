...
{#if variant === 'card'}
	{#each Array(count) as _, i}
		<div class="space-y-4 rounded-lg border p-6 {className}">
...
{:else if variant === 'post'}
	{#each Array(count) as _, i}
		<div class="space-y-3 rounded-lg border p-4 {className}">
...
{:else if variant === 'profile'}
	{#each Array(count) as _, i}
		<div class="space-y-4 rounded-lg border p-6 text-center {className}">
			<div class="{baseClasses} mx-auto h-20 w-20 rounded-full"></div>
			<div class="space-y-2">
				<div class="{baseClasses} mx-auto h-4 w-32"></div>
				<div class="{baseClasses} mx-auto h-3 w-24"></div>
			</div>
			<div class="space-y-2">
				<div class="{baseClasses} h-3 w-full"></div>
				<div class="{baseClasses} mx-auto h-3 w-4/5"></div>
			</div>
			<div class="{baseClasses} mx-auto h-8 w-24"></div>
		</div>
	{/each}
...
{:else}
	{#each Array(count) as _, i}
		<div class="{baseClasses} {className}" style="height: {height}; width: {width};"></div>
	{/each}
{/if}
