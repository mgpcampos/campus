<script lang="ts">
	...
	function getVariantClasses(item: NavigationItem, active: boolean) {
		const base = 'transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2';
...
>
	{#each items as item}
		<a
			href={item.href}
			class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background disabled:pointer-events-none disabled:opacity-50 px-3 py-2 {getVariantClasses(item, isActive(item.href))}"
		...
		>
			...
			{#if item.badge}
				<span 
					class="ml-2 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-primary-foreground bg-primary rounded-full"
